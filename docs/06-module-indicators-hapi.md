...
4. **Composite.** Weighted mean across domains → overall HAPI. v1 weights **Health and Care Access at 2×** and the other four domains at 1× (renormalized over the domains present for a jurisdiction × year); documented in `hapi_v1.py` and adjustable in a future method version.
