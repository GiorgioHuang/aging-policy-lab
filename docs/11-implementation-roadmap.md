...
- **阶段 5 — 看板与对外**:Next.js 看板可视化政策时间轴/HAPI/分析;为未来 SaaS 预留鉴权与多租户接缝。
...
- Next.js dashboards: policy timeline, HAPI by domain/jurisdiction, analytics views.
- Wire the **auth seam** and **tenancy-ready** `org_id` (nullable) per [`02-architecture.md`](02-architecture.md) §5 — without turning on multi-tenancy yet.
