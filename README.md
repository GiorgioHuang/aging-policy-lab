...
> **Status: Phase 4 — Policy Analytics + AI Research Assistant.** All five modules now have a working v1. On top of the Policy Library + HAPI (Phase 3): **Policy Analytics** computes Tier-1 descriptive trends (tagged `Association`) and a fully worked **interrupted time series** (segmented regression + HAC SEs, tagged `Causal(ITS)` with its assumptions and limitations) — the "association ≠ causation" guardrail made explicit end-to-end; and the **AI Research Assistant** turns a topic into a grounded **evidence pack** (policies + literature + findings + indicators, each cited) plus a Claude-drafted cited review. The web app surfaces everything: `/policies` · `/hapi` · `/data` · `/analytics` · `/assistant`. See [`docs/11-implementation-roadmap.md`](docs/11-implementation-roadmap.md).
>
